...
function draftTable(Selectindustry) {
  ...
  for (const info_row in obj) {
    if (Selectindustry == obj[info_row].industry || Selectindustry == 'Market') {
      ...
      Col6.innerHTML = updateSymbolBalance(obj[info_row].symbol).toFixed(2);     // Total Balance
...
function updateBalance() {
  ...
  return document.getElementById('balance').innerHTML = bal;
}
...
function currentBalance () {
  return initial_balance - history.reduce(getSum, 0);
...
function updateSymbolBalance (symbol) {
  const result = history.filter(record => record[1] == symbol);
  return result.reduce(getSum, 0);
